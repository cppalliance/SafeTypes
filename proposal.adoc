= Boost.SafeTypes: A Proposal
Matt Borland <matt@mattborland.come>
:toc:
:icons: font
...
The aim of Boost.SafeTypes is to be used as a complete replacement for the builtin numeric types of pass:[C++].
...
. No mixed sign operations
. No operators defined between Boost.SafeTypes and builtin numeric types except for explicit, and non-narrowing construction
. Any numerical error such as overflow will `throw` on error by default.
. An `std::string_view` constructor and user-defined literals for easy and correct construction
...
In many cases such as mixed sign operations we intend to emit a compiler error.
To make these compiler errors useful they must be trivially human-readable instead of emitting the template spaghetti that pass:[C++] and Boost are known for.
...
.This example shows how to use Boost.safe_numerics
====
[source, c++]
----
try
...
.An annotated version of the Example 1 except using Boost.SafeTypes instead
====
[source, c++]
----
...
try
{
    using namespace boost::safetypes;
    i8 x = INT_MAX; // Compile error: implicit narrowing is not allowed
    i8 y = 2;       // Compile error: implicit narrowing is not allowed
    i8 z = x + y;   // Good since x and y are both of type i8
}
catch(const std::exception & e)
{
    std::cout << "error detected:" << e.what() << std::endl;
...
Moving the errors from run time to compile time should be far easier to debug, and lead to fewer instances where exceptions are possible.

=== Intel Safe Arithmetic

Boost.SafeTypes offers to provide a smaller set of features (e.g. no arbitrary precision arithmetic) with greater interoperability with the STL.
...
Boost.SaftTypes aims to take this a step further in rigidity of rules.
For example, UBSan will allow rollover of an unsigned integer, whereas by default Boost.SaftTypes will throw.

== Conclusion

In conclusion, Boost.SafeTypes aims to offer a library level solution to issues of arithmetic safety in pass:[C++].
